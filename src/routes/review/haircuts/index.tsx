...
import { getMyProfile, getHaircutReviewQueue } from "@/lib/queries";
import { isAdminRole } from "@/lib/roles";
import { useAsync } from "@/lib/useAsync";
import { EmptyState, ErrorState, LoadingSkeleton, LockedState } from "@/components/aura/States";
import { Scissors, ShieldAlert, Clock, ChevronRight, User } from "lucide-react";

export const Route = createFileRoute("/review/haircuts/")({ component: HaircutReviewQueue });

function HaircutReviewQueue() {
  const profile = useAsync(getMyProfile, []);
  const admin = profile.data ? isAdminRole(profile.data.role) : false;
  const queue = useAsync(async () => (admin ? getHaircutReviewQueue() : []), [admin]);
      ...
      <div className="relative z-10">
        ...
        {profile.data && !admin && (
          <div className="p-8 rounded-3xl bg-zinc-950/80 border border-white/10 shadow-[0_0_30px_-5px_rgba(0,0,0,0.8)] backdrop-blur-xl text-center">
            ...
            <LockedState reason="Μόνο admins έχουν πρόσβαση στην ουρά αξιολόγησης κουρεμάτων." />
          </div>
        )}
          ...
          <>
            ...
            {queue.data && queue.data.length === 0 && (
              <div className="p-12 rounded-3xl bg-zinc-950/80 border border-white/10 shadow-[0_0_30px_-5px_rgba(0,0,0,0.8)] backdrop-blur-xl text-center">
                <EmptyState title="Καμία εκκρεμής υποβολή" hint="Όταν ένας student υποβάλει review κουρέματος, θα εμφανιστεί εδώ." />
              </div>
            )}
            {queue.data && queue.data.length > 0 && (
                ...
                <ul className="divide-y divide-white/5">
                  {queue.data.map((row) => (
                    <li key={row.id} className="group">
                      <Link
                        to="/review/haircuts/$haircutId"
                        params={{ haircutId: row.id }}
